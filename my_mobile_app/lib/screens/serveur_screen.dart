...
class _ServeurScreenState extends State<ServeurScreen> with TickerProviderStateMixin {
  ...
  final Color deliveredColor = const Color(0xFF9B59B6);
 ...
 Future<void> _loadNotifications() async {
  try {
    ...
    if (mounted) {
      // Filtrer pour n'afficher que les notifications pertinentes pour le serveur
      final serveurNotifications = notifications.where((notif) {
        final type = notif['type'] ?? '';
        // Le serveur voit les commandes prêtes et les commandes terminées
        return type == 'order_ready_for_delivery' || type == 'order_completed';
      }).toList();
      ...
      setState(() {
        _notifications = serveurNotifications;
        _notificationCount = serveurNotifications.length;
      });
      
      // Si de nouvelles notifications arrivent, afficher un message
      if (_notificationCount > oldCount && serveurNotifications.isNotEmpty) {
        _showNewNotificationSnackBar(serveurNotifications.first);
      }
    }
  } catch (e) {
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    if (type == 'order_ready_for_delivery') {
                      notificationColor = successColor;
                      notificationIcon = Icons.delivery_dining;
                    } else if (type == 'order_completed') {
                      notificationColor = completedColor;
                      notificationIcon = Icons.done_all;
  ...
  Future<void> _updateOrderStatus(String orderId, String newStatus) async {
    try {
      ...
      if (mounted) {
        if (success) {
          _showSnackBar('✓ Commande marquée comme ${_getStatusText(newStatus)}');
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: LinearGradient(
                      colors: [completedColor, successColor],
                    ),
                    borderRadius: BorderRadius.circular(15),
                  ),
            ...
            Container(
              ...
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  colors: [completedColor.withValues(alpha: 0.05), Colors.white],
                  begin: Alignment.topLeft,
                  end: Alignment.bottomRight,
                ),
                borderRadius: BorderRadius.circular(20),
                border: Border.all(color: completedColor.withValues(alpha: 0.2)),
              ),
              child: Column(
                children: [
                  Icon(Icons.check_circle_outline, size: 60, color: completedColor),
                  const SizedBox(height: 16),
                  const Text(
  ...
  List<dynamic> _getOrdersByStatus(String status) {
    return _orders.where((order) {
      ...
      // Pour l'onglet "À livrer", afficher les commandes prêtes (ready)
      if (status == 'livrée') {
        return orderStatus == 'prête'; // ready en français
      }
      
      // Pour l'onglet "Terminées"
      if (status == 'terminée') {
        return orderStatus == 'terminée';
      }
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [
            Colors.white,
            (statusConfig['color'] as Color).withValues(alpha: 0.05),
          ],
          begin: Alignment.topLeft,
          end: Alignment.bottomRight,
        ),
            ...
            children: [
                ...
                children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(color: (statusConfig['color'] as Color).withValues(alpha: 0.3)),
                    ),
                    child: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        Icon(statusConfig['icon'] as IconData, size: 14, color: statusConfig['color'] as Color),
              ...
              if (order['orderItems'] != null && order['orderItems'] is List && (order['orderItems'] as List).isNotEmpty)
                Container(
                  padding: const EdgeInsets.all(12),
                  decoration: BoxDecoration(
                    color: (statusConfig['color'] as Color).withValues(alpha: 0.05),
                    borderRadius: BorderRadius.circular(15),
                  ),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                        ...
                        Padding(
                          ...
                          child: Row(
                            children: [
                              ...
                              const SizedBox(width: 12),
                              Text(
                                '${(((item['product'] != null && item['product']['price'] != null ? item['product']['price'] : 0.0) as num) * (item['quantity'] ?? 1)).toStringAsFixed(2)} DT',
                                style: const TextStyle(
              ...
              // Bouton d'action (seulement pour les commandes à livrer)
              if (status == 'prête')
                SizedBox(
                  width: double.infinity,
                  child: ElevatedButton(
      ...
      body: _isLoading
          ...
          : TabBarView(
              ...
              children: [
                ...
                // Onglet 2: Terminées
                _buildOrdersList('terminée', Icons.done_all, 'Aucune commande terminée'),
              ],
            ),
    );
  }
